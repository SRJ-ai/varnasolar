...
/**
 * Solar Calculation Engine Contract
 * Calculates realistic generation, savings, subsidy, payback and environmental metrics
 */
export function calculateSolarSavings(
  ...
  // Calculate recommended system size in kW (rounded to 1 decimal, minimum 1 kW)
  let rawKw = dailyUnits / dailyGenPerKW;
  if (rawKw < 1) rawKw = 1;
  const systemSizeKW = Math.round(rawKw * 10) / 10;

  // Roof area required (~85 sq.ft per kW)
  ...
  const lifetimeGenerationKWh30Yr = Math.round(annualGenerationKWh * 30 * 0.9); // with 10% average degradation

  // Pricing & Turnkey EPC Cost
  ...
  // Central Subsidy (PM Surya Ghar for residential)
