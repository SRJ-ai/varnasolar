...
const ProjectCard = ({ project, idx, openModal }: { project: ProjectCaseStudy, idx: number, openModal: (p: ProjectCaseStudy) => void }) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col flex-grow p-6 md:p-7 gap-4" style={{ transform: "translateZ(10px)" }}>
        ...
        <div className="grid grid-cols-3 gap-3 pt-4 mt-2 border-t border-ink/10">
          ...
          <div>
            <span className="label-mono text-ink-mute block mb-1 text-[10px]">Generation</sp              {filteredProjects.map((project, idx) => (
                <ProjectCard key={project.id} project={project} idx={idx} openModal={openModal} />
              ))}t { motion, AnimatePresence, useMotionValue, useSpring, useTransform } from 'framer-motion';
import { ArrowUpRight, MapPin, Zap, IndianRupee, Leaf, Building2, X, MessageSquare } from 'lucide-react';
import { Link } from 'react-router-dom';
import { PageTransition } from '@/components/common/PageTransition';
import { PROJECTS_DATA } from '@/data/projectsData';
