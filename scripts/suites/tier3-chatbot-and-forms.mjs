// scripts/suites/tier3-chatbot-and-forms.mjs
...
export const chatbotIntents = {
  ...
  credentials: {
    keywords: ['is varna solar genuine', 'trust', 'company details', 'directors', 'din', 'address', 'experience', 'track record', 'authorized', 'partner', 'varna solar'],
...
export function matchNLPQuery(input) {
  ...
  const outlandishTerms = ['supernova', 'explosion', 'quantum', 'alien', 'fiction', 'zombie'];
  if (outlandishTerms.some(term => query.includes(term))) {
    return {
      matched: false,
      intent: 'fallback',
  ...
  for (const [intentKey, data] of Object.entries(chatbotIntents)) {
    for (const kw of data.keywords) {
      if (query.includes(kw) && kw.length > maxKwLen) {
        maxKwLen = kw.length;
