// scripts/suites/tier2-boundary-cases.mjs
// Tier 2: Boundary & Corner Cases, Input Sanitization, 38-State Matrix & Error Handling

import { describe, test, assert, assertEqual, assertNotEqual, assertGreaterThan, assertGreaterThanOrEqual, assertLessThan, assertBetween, assertMatches, assertThrows } from '../test-harness.mjs';
...
export function sanitizeAndCalculateSolar(monthlyBill, stateCode = 'TG', connectionType = 'residential') {
  ...
  const costPerKW = systemSizeKW <= 2 ? 60000 : 52000;
    ...
    test('2.6 Chatbot NLP Edge Inputs, Empty Queries & Fuzz Payloads', () => {
      ...
      function matchIntent(query) {
        ...
        for (const [intent, kws] of Object.entries(intentKeywords)) {
          for (const kw of kws) {
            if (clean.includes(kw) && kw.length > maxKwLen) {
              maxKwLen = kw.length;
              bestIntent = intent;
            }
          }
        }
        return { intent: bestIntent, confidence: maxKwLen > 0 ? 1 : 0 };
      }
